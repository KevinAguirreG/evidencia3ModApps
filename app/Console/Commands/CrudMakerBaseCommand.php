...
abstract class CrudMakerBaseCommand extends GeneratorCommand
{
	...
	protected $ignoreColumns = ["id", "created_at", "updated_at", "created_by", "updated_by", "deleted_at", "is_active"];
	protected $modelName;
	protected $dbTable;
	protected $arrFields;
...
#region database
	...
	protected function getTablefields($table = null)
	{
		...
		return DB::connection('mysql_information')
			->table('COLUMNS')
			->where(['TABLE_SCHEMA' => config('database.connections.mysql.database'), 'TABLE_NAME' => $table ?? $this->dbTable])
			->get();
	}
