...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('deliveries', function (Blueprint $table) {
            $table->increments('id');
            $table->integer('destiny_id')->unsigned()->comment('Destino');
            $table->foreign('destiny_id')->references('id')->on('destinities');
            $table->integer('delivery_status_id')->unsigned()->comment('Estatus de entregas');
            $table->foreign('delivery_status_id')->references('id')->on('delivery_statuses');
			$table->timestamp('date')->comment('Fecha de la entrega');
